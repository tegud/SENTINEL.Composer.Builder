...
function processLogEntries(hotelData, logEntries, searchId) {
	return new Promise(function(resolve) {
		...
		while(logEntries.length) {
			...
			var isAccuracyReport = currentEntry.url.indexOf('/beacon/hotelDetailsAccuracy') > -1;
			...
			if(isSearchRequest && isAccuracyReport) {
			...
			if(currentSearch && buildKey(currentSearch) === buildKey(currentEntry)) {
				...
				accuracyReports.push(new AccuracyReportWithHotelDetails(searchId, currentSearch, serverSearchContext, currentEntry, accuracyReportForSearch));
			}
			else {
		...
		if(currentSearch && !accuracyReportForSearch) {
			accuracyReports.push(new AccuracyReportWithHotelDetails(searchId, currentSearch, serverSearchContext, (buildKey(currentSearch) === buildKey(lastHotelDetails) ? lastHotelDetails : undefined), 1));
		}
...
function buildObject(logEntries, hotelInfo) {
	...
	return processLogEntries(hotelInfo, sortedLogEntries, logEntries.key);
}
