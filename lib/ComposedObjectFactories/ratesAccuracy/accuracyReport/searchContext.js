...
var searches = {
	'ks': { mapper: setQuickSearch.bind(undefined, 'keyword'), regex: /^ks([0-9]+)_([A-Z\-]+)_([A-Z\- ]+)(\-p([0-9]+))?\.aspx$/i },
	'k': { mapper: setStandardSearch.bind(undefined, 'keyword'), regex: /^k([0-9]+)_([A-Z\- ]+)\-hotels(\-p([0-9]+))?\.aspx$/i },
	'rs': { mapper: setQuickSearch.bind(undefined, 'region'), regex: /^rs([0-9]+)_([A-Z\-]+)_([A-Z\- ]+)(\-p([0-9]+))?\.aspx$/i },
	'r': { mapper: setStandardSearch.bind(undefined, 'region'), regex: /^r([0-9]+)_hotels\-in\-([A-Z\- ]+)(\-p([0-9]+))?\.aspx$/i }
};
...
module.exports = function buildSearchContext(serverSearchContext) {
	...
	try {
		...
		if(searchTypeMatch) {
			...
			var regexMatches = searches[searchType].regex.exec(url);
