...
module.exports = function(config) {
	...
	function handleMessage(msg) {
		...
		var aggregator = _.chain(aggregators).filter(function(aggregator) { return aggregator.type === parsedData.aggregatorType }).first().value();

		stores[parsedData.store.name].getSessionList(parsedData).then(function(logs) {
			var builtObject = aggregator.factory({ events: _.map(logs, function(log) { return JSON.parse(log); }) });
