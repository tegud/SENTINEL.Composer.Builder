...
function getModuleStartPromise(eventEmitter, module) {
	return module.start(eventEmitter);
}

function startUp(eventEmitter, allModules) {
	var moduleStartPromises = _.map(allModules, getModuleStartPromise.bind(undefined, eventEmitter));
...
module.exports = function(config) {
	...
	function handleMessage(parsedData) {
		stores[parsedData.store.name].getSessionList(parsedData).then(aggregators.compose.bind(undefined, parsedData)).then(function(builtObject) {
			// publish(publishers, builtObject);
			eventEmitter.emit('builtObjectComplete', builtObject);
		});
	}
